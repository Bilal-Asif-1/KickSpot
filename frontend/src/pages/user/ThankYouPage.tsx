...
export default function ThankYouPage() {
  ...
  return (
    <div className="min-h-screen bg-black">
      <div className="mx-auto max-w-4xl p-2 sm:p-6 pt-4 sm:pt-8">
        {/* Success Header */}
        <div className="text-center mb-4 sm:mb-8">
          <div className={`inline-flex items-center justify-center w-12 h-12 sm:w-20 sm:h-20 rounded-full mb-3 sm:mb-6 ${
            paymentMethod === 'cod' ? 'bg-yellow-100' : 'bg-green-100'
          }`}>
            {paymentMethod === 'cod' ? (
              <Banknote className="h-6 w-6 sm:h-10 sm:w-10 text-yellow-600" />
            ) : (
          ...
          <CardContent className="p-3 sm:p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <div>
                  <h3 className="flex items-center gap-1.5 sm:gap-2 text-gray-900 font-semibold mb-2 sm:mb-3 text-xs sm:text-base">
                  ...
                  <div className="space-y-1.5 sm:space-y-2">
                    <div>
                      ...
                      <p className="font-medium text-gray-900 text-xs sm:text-sm">{customerDetails?.name}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-600">Email</p>
                      <p className="font-medium text-gray-900 text-xs sm:text-sm">{customerDetails?.email}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-600">Phone</p>
                      <p className="font-medium text-gray-900 text-xs sm:text-sm">{customerDetails?.phone}</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="text-gray-900 text-xs sm:text-sm">{customerDetails?.address}</p>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    {items?.map((item: any, index: number) => (
                      <div key={index} className="flex items-center gap-2 sm:gap-3 p-1.5 sm:p-2 border rounded-lg">
                        {item.image_url && (
                          <img 
                        ...
                        <div className="text-right">
                          <p className="font-semibold text-gray-900 text-xs sm:text-sm">${(item.price * item.quantity).toFixed(2)}</p>
                        </div>
                      </div>
                    ))}
                    ...
                    <div className="border-t pt-2 sm:pt-3">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-bold text-gray-900 text-sm sm:text-lg">${total?.toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <div>
                  ...
                  <div className="text-center">
                    <div className="inline-flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 bg-blue-100 rounded-full mb-1.5 sm:mb-2">
                      <Truck className="h-4 w-4 sm:h-5 sm:w-5 text-blue-600" />
                    </div>
                    <h4 className="font-semibold text-gray-900 mb-1 text-xs sm:text-sm">Order Processing</h4>
                ...
                <div className="pt-3 sm:pt-4">
                  <div className="space-y-1.5 sm:space-y-2">
                    <Button 
                      ...
                      variant="outline" 
                      className="w-full border-gray-300 text-gray-700 hover:bg-gray-50 text-xs sm:text-sm py-2 sm:py-2.5"
                    >
                    ...
                    <Button 
                      onClick={() => navigate('/')} 
                      className="w-full bg-black hover:bg-gray-800 text-white text-xs sm:text-sm py-2 sm:py-2.5"
                    >
